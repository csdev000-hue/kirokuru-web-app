...
export function POST(request: Request, context: RouteContext) { return withCurrentUser(async (user, requestId) => { await readBody(request, registerCandidateSchema); const data = await registerTicketCandidate({ userId: user.id, candidateId: resourceId((await context.params).id), requestId }); return Response.json({ data, requestId }, { status: data.existing ? 200 : 201 }); })(); }
