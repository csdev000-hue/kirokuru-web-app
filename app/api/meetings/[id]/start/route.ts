...
import { startLiveMeeting } from "@/lib/services/live-meeting-service";
export function POST(request: Request, context: RouteContext) { return withCurrentUser(async (user, requestId) => {
 ...
 return Response.json({ data: await startLiveMeeting({ userId: user.id, meetingId: resourceId((await context.params).id), requestId }), requestId });
})(request); }
