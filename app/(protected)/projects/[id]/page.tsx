...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
 ...
 return <main><Link href={`/organizations/${project.organizationId}`}>組織へ</Link><h1>{project.name}</h1><p>{project.description || "説明はありません"}</p><p>状態: {project.status} · あなたの権限: {project.role}</p><Members members={members} />{project.role === "owner" && <><h2>プロジェクト設定</h2><Editor key={project.updatedAt.toISOString()} kind="projects" existing={{ id: project.id, name: project.name, description: project.description, status: project.status }} /></>}</main>;
}
