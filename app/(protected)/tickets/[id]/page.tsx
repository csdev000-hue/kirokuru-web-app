...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
 ...
 return <main><Link href={`/projects/${ticket.projectId}/tickets`}>チケット一覧へ</Link><h1>{ticket.title}</h1><p className="user-content">{ticket.description}</p><p>{ticket.status} · {ticket.priority} · {ticket.type}</p><p>担当者: {ticket.assignee?.name ?? "未設定"} · 期限: {ticket.dueDate ?? "なし"} {isOverdue(ticket.dueDate, ticket.status) && <strong>期限切れ</strong>}</p><p>作成者: {ticket.createdBy.name} · 作成: {ticket.createdAt.toISOString()} · 更新: {ticket.updatedAt.toISOString()}</p>{ticket.source && <section aria-label="作成元"><p>このチケットは会議から作成されました</p><p><Link href={`/meetings/${ticket.source.meetingId}`}>元の会議</Link></p><p><Link href={`/ticket-candidates/${ticket.source.candidateId}`}>元の候補・根拠発言</Link></p><p><Link href={`/meetings/${ticket.source.meetingId}/minutes?version=${ticket.source.minutesVersion}`}>元の議事録 Version {ticket.source.minutesVersion}</Link></p></section>}
