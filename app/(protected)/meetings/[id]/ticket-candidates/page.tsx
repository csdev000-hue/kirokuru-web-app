...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
 ...
 const pageLink=(page:number)=>`?${new URLSearchParams(Object.entries({...parsed.data,page}).map(([k,v])=>[k,String(v)]))}`;
 ...
 <nav aria-label="候補のページ"><span>Page {parsed.data.page}</span>{parsed.data.page>1&&<Link href={pageLink(parsed.data.page-1)}>前のページ</Link>}{candidates.length===parsed.data.limit&&<Link href={pageLink(parsed.data.page+1)}>次のページ</Link>}</nav>
 </main>;
}
