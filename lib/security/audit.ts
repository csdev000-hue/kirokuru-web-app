...
const auditSchema = z.object({
  ...
  action: z.enum(["organization.delete", "project.archive", "project.delete", "organization.create", "organization.update", "organization.member.add", "organization.member.remove", "project.create", "project.update", "project.member.add", "project.member.remove", "ticket.create", "ticket.update", "ticket.delete", "meeting.create", "meeting.end", "ai.minutes.generate", "ai.ticket.generate", "candidate.approve", "candidate.reject", "ticket.register"]),
  resourceType: z.enum(["organization", "project", "ticket", "meeting", "minutes", "candidate"]),
  resourceId: z.uuid().nullable(),
  metadata: z.object({
    changedFields: z.array(z.enum(["name", "description", "status", "role", "assigneeId", "dueDate", "title"])).max(20).optional(),
