...
export function logAIMetric(value: z.input<typeof metricSchema>) { console.info({ event: "ai_minutes", ...metricSchema.parse(value) }); }

const candidateMetricSchema = metricSchema.extend({ minutesId: z.uuid(), candidateCount: z.number().int().min(0).max(50) }).strict();
export function logCandidateMetric(value: z.input<typeof candidateMetricSchema>) { console.info({ event: "ai_ticket_candidate", ...candidateMetricSchema.parse(value) }); }
