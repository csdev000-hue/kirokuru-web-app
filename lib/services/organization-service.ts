...
export async function createOrganization(userId: string, input: unknown, db = getDb()) {
  ...
  return db.transaction(async (tx) => {
    ...
    await writeAuditLog({ organizationId: row.id, userId, action: "organization.create", resourceType: "organization", resourceId: row.id }, tx);
...
export async function updateOrganization(userId: string, organizationId: string, input: unknown, db = getDb()) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (!row) throw new AccessError("RESOURCE_NOT_FOUND");
    await writeAuditLog({ organizationId, userId, action: "organization.update", resourceType: "organization", resourceId: organizationId, metadata: { changedFields: ["name"] } }, tx);
...
export async function deleteOrganization(userId: string, organizationId: string, db = getDb()) {
  return db.transaction(async (tx) => {
    ...
    await writeAuditLog({ organizationId, userId, action: "organization.delete", resourceType: "organization", resourceId: organizationId }, tx);
  });
}
