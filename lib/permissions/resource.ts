...
async function authorize(input: AccessInput, resource: { id: string; projectId: string } | undefined, db: ReturnType<typeof getDb>) {
...
export async function requireTicketAccess(input: AccessInput & { ticketId: string }, db = getDb()) {
...
export async function requireMeetingAccess(input: AccessInput & { meetingId: string }, db = getDb()) {
...
export async function requireMinutesAccess(input: AccessInput & { minutesId: string }, db = getDb()) {
...
export async function requireTicketCandidateAccess(input: AccessInput & { candidateId: string }, db = getDb()) {
